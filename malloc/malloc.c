...
{
	int offset = 0;
	int slen = string_length(src) + 1;
	char *dupbuff = (char *)malloc((sizeof(char) * slen));

	while(src[offset] != '\0'){
		dupbuff[offset] = src[offset];
		offset++;
	}
...
{
	int offset = 0;
	char *srcbuff = (char *)src;
	char *destbuff = (char *) dest;
...
void *resize_memory(void *ptr, int old_size, int new_size)
{
    ...
    void *mem_area = malloc(new_size);
    if (!mem_area)// error allocating memory
    {
        return NULL;
    }

    mem_copy(mem_area, ptr, old_size);
    free(ptr);
